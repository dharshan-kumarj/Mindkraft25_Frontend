...
const MindkraftEventsPage: React.FC = () => {
  ...
  const [isInterCollege, setIsInterCollege] = useState<boolean | null>(null);
  ...
  // Check for intercollege status
  useEffect(() => {
    const intercollegeValue = Cookies.get('intercollege');
    if (intercollegeValue === 'true') {
      setIsInterCollege(true);
    } else if (intercollegeValue === 'false') {
  ...
  return (
    ...
    >
      ...
      {isInterCollege !== null && showPaymentCard && (
        <div className="fixed top-20 left-1/2 transform -translate-x-1/2 z-40 w-11/12 max-w-2xl">
          <div className="bg-gradient-to-r from-indigo-800 to-purple-900 rounded-xl shadow-2xl border border-purple-500/30 p-6 backdrop-blur-sm animate-fadeIn">
            <div className="absolute top-3 right-3">
              <button onClick={dismissPaymentCard} className="text-gray-300 hover:text-white">
            ...
            <h3 className="text-xl md:text-2xl font-bold text-center text-white mb-2">
              MINDKRAFT'25 Mandatory Payment
            </h3>
            
            <div className="text-center mb-4">
              <span className="text-3xl font-bold text-purple-200">₹{isInterCollege ? "400" : "250"}</span>
            </div>
            
            <p className="text-purple-100 text-center mb-6">
              {isInterCollege 
                ? "All external participants must pay the mandatory registration fee of ₹400 to participate in Mindkraft 2025 events."
                : "All Karunya students must pay the mandatory registration fee of ₹250 to participate in Mindkraft 2025 events."
              }
            </p>
            ...
            <div className="flex justify-center">
              <button 
                ...
                className="px-6 py-3 bg-gradient-to-r from-purple-500 to-indigo-600 hover:from-purple-600 hover:to-indigo-700 text-white font-medium rounded-lg transition shadow-lg hover:shadow-purple-500/50 transform hover:scale-105"
              >
                Make Payment Now
              </button>
            </div>
